...
/* befindet sich im Lib Verzeichnis. F�r die Anpassung der Schnitt-		*/
/* stelle muss der Wert (_LCDMenuLib_cfg_lcd_type) ver�ndert werden.    */
/************************************************************************/
...
{	
	...
	do {
		//�berpr�fen ob das Element Kinder hat
		if(tmp=search->getChild(0)) 
		{
			//Kinder gefunden, daher kann Enter gedr�ckt werden
			Button_enter();

			//�berpr�fen ob �bereinstimmung vorhanden ist
			...
			//Rekrosiv Funktion erneut aufruf um eine Ebene tiefer zu �berpr�fen
			found = selectElementDirect(*tmp, p_search);
			
			//Erneut Pr�fen ob eine �bereinstimmung vorhanden ist
			if(found == 1) 
			{
				// gefunden
				break; 
			...
			{
				// nicht gefunden, wieder eine Ebene nach oben und n�chstes Element ausw�hlen
				Button_quit();
		...
		{
			// Keine Kinder gefunden, trotzdem �berpr�fen ob �bereinstimmung vorhanden ist
...
void	LCDMenuLib::Button_quit(uint8_t opt)
{
  ...
  { 
	  // Splash Screen �berpr�fen, muss vor goBack eingebunden werden
